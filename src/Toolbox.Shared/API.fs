...
let updateBoard (board:Board) (possibilities:Possibilities) (row:int) (col:int) value : Board * Possibilities =
    // if value |> Option.isNone then board, possibilities
    // else
    /// issue is somewhere here - it's not updating the board correctly when removing or deleting value
        ...
        let updatedPossibilities = Array.init 9 (fun i -> Array.init 9 (fun j -> 
            ...
            else if i = row || j = col || (i / 3 = row / 3 && j / 3 = col / 3) then Set.remove value.Value possibilities.[i].[j]
